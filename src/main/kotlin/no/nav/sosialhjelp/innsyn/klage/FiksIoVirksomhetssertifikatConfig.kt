...
class FiksIoVirksomhetssertifikatConfig(
    ...
    fun virksomhetssertifikatConfig(): VirksomhetssertifikatKonfigurasjon {
        ...
        val keyStorePassword = UUID.randomUUID().toString()
        keyStore.load(sertifikat.data.newInput(), keyStorePassword.toCharArray())
        return VirksomhetssertifikatKonfigurasjon.builder().keyStore(KeyStore.getInstance("pkcs12")).keyStorePassword(keyStorePassword).keyAlias(passwordThingy.alias).keyPassword(passwordThingy.password).build()
    }
