...
interface RedisService {
    ...
    fun put(
        key: String,
        value: ByteArray,
        timeToLiveSeconds: Long = defaultTimeToLiveSeconds,
    )

    fun delete(key: String)

    companion object {
        fun <T : Any> pakkUtRedisData(
            bytes: ByteArray?,
            requestedClass: Class<out T>,
            key: String,
            log: Logger,
        ...
        private fun valider(obj: Any?) {
            when {
                obj is DigisosSak && obj.sokerFnr != getUserIdFromToken() -> throw DigisosSakTilhorerAnnenBrukerException(
                    "DigisosSak tilhører annen bruker",
                )
                ...
                obj is JsonSoknad && obj.additionalProperties.isNotEmpty() -> throw IOException(
                    "JsonSoknad har ukjente properties - må tilhøre ett annet objekt. Cache-value tas ikke i bruk",
                )
