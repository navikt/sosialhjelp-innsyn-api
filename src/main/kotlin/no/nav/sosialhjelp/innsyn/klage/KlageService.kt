...
class KlageServiceImpl(
    ...
    override fun sendKlage(fiksDigisosId: String, klage: InputKlage, token: String) {
        val (klient, konto) = hentKlientOgKonto(fiksDigisosId, token, "no.nav.sosialhjelp.klage.v1")

        konto?.let {
            klient.send(MeldingRequest.builder().mottakerKontoId(it.kontoId).build(), lagKlageFil(klage), "klage.json")
        } ?: error("Kunne ikke sende klage")
    }
    ...
    override fun hentKlager(fiksDigisosId: String, token: String): List<Klage> {
        val (klient, konto) = hentKlientOgKonto(fiksDigisosId, token, "digisos.klage.hent")

        val sendtMelding = konto?.let {
            klient.send(MeldingRequest.builder().mottakerKontoId(it.kontoId).build(), fiksDigisosId, "???")
        } ?: error("Kunne ikke sende til Fiks IO")

        return runBlocking {
            withTimeout(2000) {
                waitForResult(klient, sendtMelding)
            }.catch { e -> log.error("Fikk feil i henting av klager", e) }.toList()
        }
    }

    private fun hentKlientOgKonto(fiksDigisosId: String, token: String, protokoll: String): Pair<FiksIOKlient, Konto?> {
        ...
        val navEnhetId = digisosSak.tilleggsinformasjon?.enhetsnummer?.let {
            ...
            Identifikator(IdentifikatorType.ORG_NO, it)
        } ?: error("Sak mangler enhetsnummer")

        val fiksIOKlientFactory = FiksIOKlientFactory(fiksIOConfig).apply {
        ...
        val fiksIOKlient = fiksIOKlientFactory.runCatching { build() }.onFailure {
