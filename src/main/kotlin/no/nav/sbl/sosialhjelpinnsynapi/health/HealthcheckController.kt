...
class SelftestController {
    ...
    val isAlive: String
        @ResponseBody
        @RequestMapping(value = ["/isAlive"], produces = [MediaType.TEXT_PLAIN_VALUE], method = [RequestMethod.GET])
        get() = APPLICATION_LIVENESS

    val isReady: String
        @ResponseBody
        @RequestMapping(value = ["/isReady"], produces = [MediaType.TEXT_PLAIN_VALUE], method = [RequestMethod.GET])
