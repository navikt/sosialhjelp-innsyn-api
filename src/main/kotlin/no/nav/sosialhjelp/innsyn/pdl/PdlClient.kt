...
class PdlClient (
    ...
    suspend fun getPersonByIdent(ident: String, token: String): PdlPerson = pdlGraphQlClientBuilder
        .buildClient(token)
    ...
    @Cacheable("pdl")
    @CircuitBreaker(name = "pdl")
    suspend fun getIdentsByIdent(ident: String, token: String): List<String> = pdlGraphQlClientBuilder
        .buildClient(token)
